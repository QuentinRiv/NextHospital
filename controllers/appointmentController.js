...
import { getPatientInfo, getDoctorInfo } from "./dbController.js"; // Ajustez le chemin relatif selon votre structure de répertoire

export async function getAppointments(key, profileId) {
  const appointments = await Appointment.find({ [key]: profileId }).sort(
    "date"
  ); // Selecte tous les RDV d'un patient, trié par date
  ...
  for (let i = 0; i < appointments.length; i++) {
    ...
    const docinfo = await getDoctorInfo("_id", appointments[i].doctor);
...
export async function create(req, res) {
  try {
    ...
    const doctor = await getDoctorInfo("name", req.body.doctorName);
